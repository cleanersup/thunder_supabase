-- Migration: Recreate get_employee_shift_details function with better error handling
-- Fixes issue where appointment was returning null

DROP FUNCTION IF EXISTS public.get_employee_shift_details(uuid, uuid);
...
BEGIN
  ...
  -- Step 2: Get appointment if exists (with explicit exception handling)

...
const handler = async (req: Request): Promise<Response> => {
  return await Sentry.withScope(async (scope) => {
    ...
    try {
      ...
      const isAppReview =
        isAppReviewEmployeePhone(lookupPhone) &&
        String(otpCode).trim() === APP_REVIEW_OTP_CODE;

      // Rate limiting: Check total failed attempts in the last hour
      // App Review demo phone is exempt so reviewers are never locked out.
      if (!isAppReviewEmployeePhone(lookupPhone)) {
      ...
      if (isAppReview) {
        // Fixed OTP for App Store Review — no SMS required.
        console.log(`🍎 App Review OTP accepted for ${lookupPhone}`);
        const { data: reviewEmployee, error: reviewEmpErr } = await supabase
          .from('employees')
          .select('id')
        ...
        if (reviewEmpErr || !reviewEmployee) {
          console.error('App Review employee missing:', reviewEmpErr);
          return new Response(
            JSON.stringify({
              error: 'No active employee found with this phone number',
              verified: false,
            }),
        ...
        // Best-effort: mark any matching unused OTP rows verified
        await supabase
          .from('otp_codes')
          .update({ verified: true })
          .eq('employee_id', employeeId)
          .eq('otp_code', APP_REVIEW_OTP_CODE)
          .eq('verified', false);
      } else {
        ...
        employeeId = otpRecord.employee_id as string;
      }

...
interface ConversionPayload {
  jobId: string;
  sourceType: "estimate" | "walkthrough";
  sourceId: string;
  operation?: string;
}
...
function formatAddress(job: JobRow): string {
  ...
  return `${job.property_street || "N/A"}${apt}, ${job.property_city || "N/A"}, ${job.property_state || "N/A"} ${job.property_zip || "N/A"}`;
}
...
serve(async (req) => {
  ...
  try {
    const { jobId, sourceType, sourceId } = await req.json() as ConversionPayload;
    if (!jobId || !sourceType || !sourceId) {
      return new Response(JSON.stringify({ error: "jobId, sourceType and sourceId are required" }), {
        status: 400,
    ...
    const sourceLabel = sourceType === "estimate" ? "estimate" : "walkthrough";
    ...
    const details = `
      ...
      <p><strong>Property:</strong> ${formatAddress(row)}</p>
      <p><strong>Total:</strong> $${(row.total_amount ?? 0).toFixed(2)}</p>
      <p><strong>Converted From:</strong> ${sourceLabel} (${sourceId})</p>
    `;

    const ownerSubject = `Job created from ${sourceLabel} — ${row.job_number || row.id}`;
    const ownerBody = wrapEmail(
      "Source converted to job",
      `<p>${sourceLabel.charAt(0).toUpperCase() + sourceLabel.slice(1)} converted to job for <strong>${clientName}</strong>.</p>${details}`,
    );

    await sendEmailViaSMTP(ownerEmail, ownerSubject, ownerBody, ownerEmail);

    if (clientEmail) {
      const clientSubject = `Your ${sourceLabel} was moved to a job with ${companyName}`;
      const clientBody = wrapEmail(
        "Your service has moved to the next step",
        `<p>Hi ${clientName},</p><p>Your ${sourceLabel} was converted to a job in our system.</p>${details}`,
      );

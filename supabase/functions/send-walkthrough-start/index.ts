...
interface WalkthroughStartRequest {
  walkthroughId: string;
}

// Date-only strings (YYYY-MM-DD) are interpreted as UTC midnight, causing day shift when
// formatted in user timezone (e.g. America/New_York). Parse and use midday UTC to avoid this.
...
<title>Walkthrough Started</title>
</head>
...
🎉 Good news! The walkthrough has started for ${contactInfo.full_name || 'the client'}!
</p>
</div>

<p style="font-size:14px;line-height:1.6;margin:15px 0">
  Your team has begun the walkthrough process. Here are the details:
</p>

<div style="background:#f8fafc;padding:15px;border-radius:8px;margin:20px 0">
  <h2 style="margin:0 0 12px;font-size:16px;color:#1e293b">Walkthrough Information</h2>
  ...
  <table style="width:100%;border-collapse:collapse">
    ...
    <tr>
      ...
      <td style="padding:8px 0;font-size:13px;font-weight:600">${formatDate(walkthrough.scheduled_date, timezone)}</td>
    </tr>
    <tr>
      <td style="padding:8px 0;font-size:13px;color:#64748b">Time:</td>
      <td style="padding:8px 0;font-size:13px;font-weight:600">${formatTime(walkthrough.scheduled_time)}</td>
    </tr>
  </table>
</div>
...
<p style="margin:5px 0">© ${new Date().getFullYear()} ${companyInfo.company_name || 'Company Name'}. All rights reserved.</p>
</div>
...
🎉 Good news! The walkthrough has started!!
</p>
</div>

<p style="font-size:14px;line-height:1.6;margin:15px 0">
  Our team has just begun your walkthrough appointment. We're excited to assess your space and provide you with the best service possible!
</p>

<div style="background:#f8fafc;padding:15px;border-radius:8px;margin:20px 0">
  <h2 style="margin:0 0 12px;font-size:16px;color:#1e293b">Appointment Details</h2>
...
📞 If you have any questions or concerns during the walkthrough, please don't hesitate to contact us at ${companyInfo.company_phone || 'Company Phone Number'}.
</p>
</div>

<p style="font-size:14px;line-height:1.6;margin:20px 0">
  Thank you for choosing ${companyInfo.company_name || 'Company Name'}!
</p>
...
<p style="margin:5px 0">
  Questions? Contact us at ${companyInfo.company_phone || 'Company Phone Number'}
</p>
</div>
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { walkthroughId }: WalkthroughStartRequest = await req.json();
    ...
    console.log('=== WALKTHROUGH START EMAIL PROCESS ===');
    ...
    const responseMessage = clientEmailSent
      ? 'Start emails sent successfully'
      : ownerEmailSent
    ...
    console.error('Error in send-walkthrough-start:', error);
    return new Response(
